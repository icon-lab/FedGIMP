...
def make_variables(layer_shape, layer_dtype, initializer):
    return (tf.Variable(initializer(shape=layer_shape, dtype=layer_dtype)))
...
class Network:
    ...
    def get_output_for(self, *in_expr: TfExpression, return_as_list: bool = False, **dynamic_kwargs) -> Union[TfExpression, List[TfExpression]]:
        ...
        with tfutil.absolute_variable_scope(self.scope, reuse=True), tf.compat.v1.keras.backend.name_scope(self.name):
            assert tf.compat.v1.get_variable_scope().name == self.scope
            ...
            for expr, name, shape in zip(in_expr, self.input_names, self.input_shapes):
                if expr is not None:
                    ...
                    expr = tf.zeros([tf.compat.v1.shape(valid_inputs[0])[0]] + shape[1:], name=name)
    ...
    def __setstate__(self, state: dict) -> None:
        ...
        my_str = self._build_module_src[0:58] + ".compat.v1 " + self._build_module_src[59:len(self._build_module_src)]
        
        self._build_module_src = my_str
    ...
    def print_trainables(self) -> None:
        """Copy the values of all trainable variables from the given network, including sub-networks."""
