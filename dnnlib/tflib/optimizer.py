...
class Optimizer:
    ...
    def __init__(self,
                 ...
                 # tf_optimizer: str = "tf.compat.v1.train.RMSPropOptimizer",
                 # tf_optimizer: str = "tfa.optimizers.RectifiedAdam",
    ...
    def register_gradients(self, loss: TfExpression, trainable_vars: Union[List, dict]) -> None:
        ...
        with tf.compat.v1.keras.backend.name_scope(self.id + "_grad"), tf.compat.v1.device(dev):
    ...
    def apply_updates(self) -> tf.Operation:
        ...
        with tfutil.absolute_name_scope(self.scope):
            ...
            for dev_idx, dev in enumerate(devices):
                with tf.compat.v1.keras.backend.name_scope("ProcessGrads%d" % dev_idx), tf.compat.v1.device(dev):
                    sums = []
            ...
            if len(devices) > 1:
                with tf.compat.v1.keras.backend.name_scope("SumAcrossGPUs"), tf.compat.v1.device(None):
            ...
            for dev_idx, (dev, grads) in enumerate(dev_grads.items()):
                with tf.compat.v1.keras.backend.name_scope("ApplyGrads%d" % dev_idx), tf.compat.v1.device(dev):
                    # Scale gradients as needed.
                    if self.use_loss_scaling or total_grads > 1:
                        with tf.compat.v1.keras.backend.name_scope("Scale"):
                            coef = tf.constant(np.float32(1.0 / total_grads), name="coef")
                            coef = self.undo_loss_scaling(coef)
                            grads = [(g * coef, v) for g, v in grads]

                    # Check for overflows.
                    with tf.compat.v1.keras.backend.name_scope("CheckOverflow"):
                        grad_ok = tf.compat.v1.reduce_all(tf.stack([tf.compat.v1.reduce_all(tf.math.is_finite(g)) for g, v in grads]))

                    # Update weights and adjust loss scaling.
                    with tf.compat.v1.keras.backend.name_scope("UpdateWeights"):
                        # pylint: disable=cell-var-from-loop
                        opt = self._dev_opt[dev]
                        ls_var = self.get_loss_scaling_var(dev)

                        if not self.use_loss_scaling:
                            ops.append(tf.compat.v1.cond(grad_ok, lambda: opt.apply_gradients(grads), tf.no_op))
                        else:
                    ...
                    if dev == devices[-1]:
                        with tf.compat.v1.keras.backend.name_scope("Statistics"):
    ...
    def get_loss_scaling_var(self, device: str) -> Union[tf.compat.v1.Variable, None]:
