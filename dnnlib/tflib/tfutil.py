...
TfExpression = Union[tf.Tensor, tf.compat.v1.Variable, tf.Operation]
...
def is_tf_expression(x: Any) -> bool:
    ...
    return isinstance(x, (tf.Tensor, tf.compat.v1.Variable, tf.Operation))
...
def flatten(x: TfExpressionEx) -> TfExpression:
    ...
    with tf.compat.v1.keras.backend.name_scope("Flatten"):
        return tf.reshape(x, [-1])


def log2(x: TfExpressionEx) -> TfExpression:
    """Logarithm in base 2."""
    with tf.compat.v1.keras.backend.name_scope("Log2"):
        return tf.log(x) * np.float32(1.0 / np.log(2.0))


def exp2(x: TfExpressionEx) -> TfExpression:
    """Exponent in base 2."""
    with tf.compat.v1.keras.backend.name_scope("Exp2"):
        return tf.exp(x * np.float32(np.log(2.0)))


def lerp(a: TfExpressionEx, b: TfExpressionEx, t: TfExpressionEx) -> TfExpressionEx:
    """Linear interpolation."""
    with tf.compat.v1.keras.backend.name_scope("Lerp"):
        return a + (b - a) * t


def lerp_clip(a: TfExpressionEx, b: TfExpressionEx, t: TfExpressionEx) -> TfExpression:
    """Linear interpolation with clip."""
    with tf.compat.v1.keras.backend.name_scope("LerpClip"):
        return a + (b - a) * tf.clip_by_value(t, 0.0, 1.0)


def absolute_name_scope(scope: str) -> tf.compat.v1.keras.backend.name_scope:
    """Forcefully enter the specified name scope, ignoring any surrounding scopes."""
    return tf.compat.v1.keras.backend.name_scope(scope + "/")
...
def convert_images_to_uint8(images, drange=[-1,1], nchw_to_nhwc=False, shrink=1):
    ...
    if shrink > 1:
        ...
        images = tf.compat.v1.nn.avg_pool(images, ksize=ksize, strides=ksize, padding="VALID", data_format="NCHW")
    if nchw_to_nhwc:
