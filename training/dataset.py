...
######################################################################################################################################################################
#PARSE INDIVIDUAL IMAGES FROM TFRECORDS FILES

def parse_tfrecord_tf(record):
    features = tf.compat.v1.parse_single_example(record, features={
        'shape': tf.io.FixedLenFeature([3], tf.int64),
        'data': tf.io.FixedLenFeature([], tf.compat.v1.string)})
...
######################################################################################################################################################################
#DATASET CLASS TO LOAD FROM UINT8 TYPE TFRECORDS FILES
class TFRecordDataset:
    def __init__(self,
        ...
        with tf.compat.v1.keras.backend.name_scope('Dataset'), tf.compat.v1.device('/cpu:0'):
    ...
    def get_random_labels_tf(self, minibatch_size): # => labels
        if self.label_size > 0:
            with tf.compat.v1.device('/cpu:0'):
                return tf.compat.v1.gather(self._tf_labels_var, tf.random.uniform([minibatch_size], 0, self._np_labels.shape[0], dtype=tf.int32))
...
def load_dataset(class_name='training.dataset.TFRecordDataset', data_dir=None, verbose=False, max_label_size=None, repeat = True, shuffle_mb=4096):
