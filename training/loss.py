...
def D_wgan(G, D, opt, training_set, minibatch_size, reals, labels, # pylint: disable=unused-argument
    ...
    with tf.compat.v1.keras.backend.name_scope('EpsilonPenalty'):
...
def D_wgan_gp(G, D, opt, training_set, minibatch_size, reals, labels, # pylint: disable=unused-argument
    ...
    with tf.compat.v1.keras.backend.name_scope('GradientPenalty'):
        ...
        mixed_norms = tf.sqrt(tf.compat.v1.reduce_sum(tf.square(mixed_grads), axis=[1,2,3]))
...
def G_logistic_saturating(G, D, opt, training_set, minibatch_size): # pylint: disable=unused-argument
    ...
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    fake_scores_out = fp32(D.get_output_for(fake_images_out, labels, is_training=True))
    loss = -tf.compat.v1.nn.softplus(fake_scores_out)  # log(1 - logistic(fake_scores_out))
    return loss
...
def G_logistic_nonsaturating(G, D, opt, training_set, minibatch_size): # pylint: disable=unused-argument
    ...
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    fake_scores_out = fp32(D.get_output_for(fake_images_out, labels, is_training=True))
    loss = tf.compat.v1.nn.softplus(-fake_scores_out)  # -log(logistic(fake_scores_out))
    return loss
...
def D_logistic(G, D, opt, training_set, minibatch_size, reals, labels): # pylint: disable=unused-argument
    ...
    fake_scores_out = autosummary('Loss/scores/fake', fake_scores_out)
    loss = tf.compat.v1.nn.softplus(fake_scores_out)  # -log(1 - logistic(fake_scores_out))
    loss += tf.compat.v1.nn.softplus(-real_scores_out)  # -log(logistic(real_scores_out)) # temporary pylint workaround # pylint: disable=invalid-unary-operand-type
    return loss
...
def D_logistic_simplegp(G, D, opt, training_set, minibatch_size, reals, labels, r1_gamma=10.0, r2_gamma=0.0): # pylint: disable=unused-argument
    ...
    if r1_gamma != 0.0:
        with tf.compat.v1.keras.backend.name_scope('R1Penalty'):
            real_loss = opt.apply_loss_scaling(tf.compat.v1.reduce_sum(real_scores_out))
            real_grads = opt.undo_loss_scaling(fp32(tf.compat.v1.gradients(real_loss, [reals])[0]))
            r1_penalty = tf.compat.v1.reduce_sum(tf.square(real_grads), axis=[1,2,3])
            r1_penalty = autosummary('Loss/r1_penalty', r1_penalty)
        loss += r1_penalty * (r1_gamma * 0.5)

    if r2_gamma != 0.0:
        with tf.compat.v1.keras.backend.name_scope('R2Penalty'):
            fake_loss = opt.apply_loss_scaling(tf.compat.v1.reduce_sum(fake_scores_out))
            fake_grads = opt.undo_loss_scaling(fp32(tf.compat.v1.gradients(fake_loss, [fake_images_out])[0]))
            r2_penalty = tf.compat.v1.reduce_sum(tf.square(fake_grads), axis=[1,2,3])
