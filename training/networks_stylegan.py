...
def _blur2d(x, f=[1,2,1], normalize=True, flip=False, stride=1):
    ...
    f = tf.compat.v1.constant(f, dtype=x.dtype, name='filter')
    strides = [1, 1, stride, stride]
    x = tf.compat.v1.nn.depthwise_conv2d(x, f, strides=strides, padding='SAME', data_format='NCHW')
...
def _downscale2d(x, factor=2, gain=1):
    ...
    return tf.compat.v1.nn.avg_pool(x, ksize=ksize, strides=ksize, padding='VALID', data_format='NCHW')
...
def pixel_norm(x, epsilon=1e-8):
    with tf.compat.v1.variable_scope('PixelNorm'):
        epsilon = tf.compat.v1.constant(epsilon, dtype=x.dtype, name='epsilon')
        return x * tf.math.rsqrt(tf.compat.v1.reduce_mean(tf.square(x), axis=1, keepdims=True) + epsilon)
...
def instance_norm(x, epsilon=1e-8):
    ...
    with tf.compat.v1.variable_scope('InstanceNorm'):
        ...
        x -= tf.compat.v1.reduce_mean(x, axis=[2,3], keepdims=True)
        epsilon = tf.compat.v1.constant(epsilon, dtype=x.dtype, name='epsilon')
        x *= tf.math.rsqrt(tf.compat.v1.reduce_mean(tf.square(x), axis=[2,3], keepdims=True) + epsilon)
        x = tf.cast(x, orig_dtype)
        return x
...
def minibatch_stddev_layer(x, group_size=4, num_new_features=1):
    with tf.compat.v1.variable_scope('MinibatchStddev'):
        ...
        y -= tf.compat.v1.reduce_mean(y, axis=0, keepdims=True)           # [GMncHW] Subtract mean over group.
        y = tf.compat.v1.reduce_mean(tf.square(y), axis=0)                # [MncHW]  Calc variance over group.
        y = tf.sqrt(y + 1e-8)                                   # [MncHW]  Calc stddev over group.
        y = tf.compat.v1.reduce_mean(y, axis=[2,3,4], keepdims=True)      # [Mn111]  Take average over fmaps and pixels.
        y = tf.compat.v1.reduce_mean(y, axis=[2])                         # [Mn11] Split channels into c channel groups
    ...
    **kwargs):                                      # Arguments for sub-networks (G_mapping and G_synthesis).
    ...
    if dlatent_avg_beta is not None:
        with tf.compat.v1.variable_scope('DlatentAvg'):
            batch_avg = tf.compat.v1.reduce_mean(dlatents[:, 0], axis=0)
    ...
    if style_mixing_prob is not None:
        with tf.compat.v1.name_scope('StyleMix'):
            ...
            layer_idx = tf.range(num_layers)[tf.newaxis, :, tf.newaxis]
            cur_layers = num_layers - tf.cast(lod_in, tf.int32) * 2
            mixing_cutoff = tf.compat.v1.cond(
...
def G_mapping(
    ...
    **_kwargs):                             # Ignore unrecognized keyword args.
    ...
    act, gain = {'relu': (tf.compat.v1.nn.relu, np.sqrt(2)), 'lrelu': (leaky_relu, np.sqrt(2))}[mapping_nonlinearity]
...
def G_synthesis(
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    act, gain = {'relu': (tf.compat.v1.nn.relu, np.sqrt(2)), 'lrelu': (leaky_relu, np.sqrt(2))}[nonlinearity]
    ...
    **_kwargs):                         # Ignore unrecognized keyword args.
    ...
    if label_size:
        with tf.compat.v1.variable_scope('LabelSwitch'):
            scores_out = tf.compat.v1.reduce_sum(scores_out * labels_in, axis=1, keepdims=True)
